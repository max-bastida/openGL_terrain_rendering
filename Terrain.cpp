...
GLuint vaoID;
GLuint theProgram;
GLuint mvpMatrixLoc, eyeLoc, lightLoc, waterHeight, tickLoc, magLoc, freqLoc, speedLoc;
...
//--------Load shaders----------------------
	...
	magLoc = glGetUniformLocation(program, "wave_mag");
	freqLoc = glGetUniformLocation(program, "freq");
	speedLoc = glGetUniformLocation(program, "speed");

	glUniform3fv(lightLoc, 1, &lightPos[0]);
	glUniform1f(waterHeight, 2.0f);
	glUniform1f(magLoc, 0.1f);
	glUniform1f(freqLoc, 60.0f);
	glUniform1f(speedLoc, 0.001f);

	GLuint texLocH = glGetUniformLocation(program, "heightMap");
	glUniform1i(texLocH, 0);
	GLuint texLocW = glGetUniformLocation(program, "waterTex");
	glUniform1i(texLocW, 1);
	GLuint texLocG = glGetUniformLocation(program, "grassTex");
	glUniform1i(texLocG, 2);
	GLuint texLocR = glGetUniformLocation(program, "rockTex");
	glUniform1i(texLocR, 3);
	GLuint texLocS = glGetUniformLocation(program, "snowTex");
	glUniform1i(texLocS, 4);
...
void display()
{
	...
	glm::mat4 view = lookAt(glm::vec3(eye_x, eye_y, eye_z), glm::vec3(look_x, look_y, look_z), glm::vec3(0.0, 1.0, 0.0)); //view matrix
